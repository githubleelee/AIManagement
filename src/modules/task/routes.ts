 ...
 *
 * 端点 30（`PUT /tasks/:taskId/sensitivity`）：US-02 集成后由 M3 的
 * `src/modules/authz/plugin.ts` 统一提供（语义与副作用同端点 23），本模块不再注册，
 * 否则会与 M3 触发 Fastify `FST_ERR_DUPLICATED_ROUTE` 使整个应用无法构建。
